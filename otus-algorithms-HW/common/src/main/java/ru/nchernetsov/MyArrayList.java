...
public class MyArrayList<T> extends AbstractList<T> implements List<T> {
    ...
    private static final float SIZE_FACTOR = (float) ((1 + sqrt(5)) / 2.0f);
    ...
    public boolean add(T t) {
        if (busySize + 1 > size) {  // используем мультипликативную схему реаллокации памяти
            if (size > MAX_SIZE / SIZE_FACTOR) {
                size = MAX_SIZE;
            } else {
                if (size == 0) {
                    size = 1;
                } else {
                    int newSize = (int) (size * SIZE_FACTOR);
                    if (newSize == size) {  // т.е. SIZE_FACTOR задан такой, что size не увеличивается
                        size += 1;            // (например size = 2 и SIZE_FACTOR = 1.2)
                    } else {
                        size = newSize;
                    }
                }
